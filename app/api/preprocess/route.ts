...
// `/api/news` and `/api/gemini`. The preprocessing step has been removed and
// the current pipeline expects `/api/gemini` to follow `/api/news`.
//
...
export async function POST(_request: NextRequest) {
  return NextResponse.json(
    { error: "Preprocessing removed. Call /api/gemini after /api/news." },
    { status: 410 }
  );
}
